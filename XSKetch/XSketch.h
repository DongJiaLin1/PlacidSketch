 ...
 * 
 * XSketch is a two-stage Sketch for network traffic anomaly detection
 */
...
// Maximum counter values at each level
const int max_range[] = {2, 14, 65534};

// Bit width of counters at each level
const int counter_bit[] = {2, 4, 16};

/**
 * @brief Slot class for Stage2 hash buckets
 */
...
/**
 * @brief Stage1: TowerSketch for flow frequency recording
 */
template<typename ID_TYPE>
class Stage1 {
public: 
...
/**
 * @brief Stage2: Hash bucket structure for precise detection
 */
template<typename ID_TYPE>
class Stage2 {
public: 
...
/**
 * @brief XSketch: Two-stage Sketch data structure
 */
template<typename ID_TYPE>
class XSketch {
public: 
